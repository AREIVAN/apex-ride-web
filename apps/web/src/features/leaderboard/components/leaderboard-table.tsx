  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className="space-y-2 p-3 sm:hidden">
        {rows.map((row) => (
          <article key={`${row.segmentId}-${row.riderId}`} className="rounded-xl border border-slate-200/80 bg-white px-3 py-2.5">
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm font-semibold text-slate-900">#{row.rank} {row.riderName}</p>
              <p className="text-sm font-bold text-brand-900">{formatDuration(row.elapsedTimeSec)}</p>
            </div>
            <p className="mt-1 text-xs text-slate-500">Intento: {new Date(row.recordedAt).toLocaleDateString()}</p>
          </article>
        ))}
      </div>

      <div className="table-scroll -mx-1 hidden px-1 sm:mx-0 sm:block sm:px-0">
